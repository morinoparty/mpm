...
各フィールドは省略可能（`null`）で、省略された場合は`mpm.json`の`settings`（グローバル設定）の値が使用されます。

<Callout type="info" title="優先順位">
- メタデータで設定が指定されている場合：その値を使用
- メタデータで`null`の場合：`mpm.json`のグローバル設定を使用

この仕組みにより、プロジェクト全体のデフォルト設定を保ちつつ、個別プラグインごとに設定を上書きできます。
</Callout>
