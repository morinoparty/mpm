...
<Callout type="info" title="管理外プラグインはライブスキャンで判定されます">
「管理外のプラグイン」は、`mpm init` を実行した時点のスナップショットではなく、その場で `plugins/` ディレクトリを走査した結果です。
init の後に手動で置かれた JAR も検出され、削除された JAR は表示されません。
`plugins/mpm/local/` に置いた JAR は走査の対象外です。`mpm list --unmanaged` と同じ判定を使っています。
</Callout>
...
- [ライフサイクル管理コマンド](/docs/commands/plugin-management/lifecycle) - `mpm list` のフィルタと表示される状態
