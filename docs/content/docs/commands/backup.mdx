...
プラグインの更新前に自動でバックアップが作成されるため、問題が発生した場合に以前の状態に戻すことができます。
...
`mpm install` / `mpm add` / `mpm adopt` などのインストール経路は、現在バックアップを作成しません。
インストール前にバックアップを取りたい場合は `/mpm backup create` を実行してください。
