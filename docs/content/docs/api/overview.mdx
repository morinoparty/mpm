---
title: API概要
description: mpmの公開APIについて
---

mpmは、他のプラグインやアドオンから利用できる公開APIを提供しています。
このAPIを使用することで、プラグインの管理機能をプログラムから制御することができます。

## API取得方法

mpmのAPIは、`MpmAPI`インターフェースを通じて提供されます。
...
// プラグインインスタンスを取得
val mpmPlugin = Bukkit.getPluginManager().getPlugin("mpm")
...
また、`plugin.yml`または`paper-plugin.yml`でmpmを依存関係として指定してください：
...
- `DownloaderRepository.downloadLatest`は非推奨です。リポジトリファイルの`versionMatcher`を無視してプラットフォーム側の最新版を返すため、対象バージョンを解決したうえで`downloadByVersion`を呼び出してください。
