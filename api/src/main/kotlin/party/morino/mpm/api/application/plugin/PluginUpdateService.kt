...
interface PluginUpdateService {
     ...
     * 指定プラグインを更新する
     *
     * 更新対象プラグインに同期している `sync:` プラグイン（子・孫）があれば、
     * 親の更新後バージョンに追従して連動更新する。多段の `sync:` でも親に近い順に伝播する。
     * 戻り値のリストは先頭が親の更新結果、以降が連動更新した子孫の結果となる。
     * ロック中の子は更新せず、`skipped = true` の結果として返す。
     * ロック・失敗・メタデータ破損で据え置かれた中間ノードの子孫は追従の対象から外れ、
     * 「同期先が更新されなかったため」という `skipped = true` の結果として返す
