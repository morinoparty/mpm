 ...
 *
 * 注意（mineauth-api 0.3.6 の仕様）:
 * `@Authenticated(permission = ...)` はユーザープリンシパルにのみ評価される。
 * `callers` に含まれるサービストークンはパーミッションチェックを完全にバイパスするため、
 * サービストークンにとっては `callers` リストが唯一のアクセス制御となる。
 * つまり、この読み書き分割で保護されるのはユーザートークンのみである。
 */
